...
import jax.random
from data.dataset import generator, random_crop_batch
...
def train():
    ...
    with tqdm(total=trainer_configs['total_steps']) as pbar:
        ...
        for steps in range(finished_steps + 1, 1000000):
            key, train_step_key, crop_size_rng,crop_rng = jax.random.split(key, num=4)
            train_step_key = shard_prng_key(train_step_key)
            batch = next(dl)

            # Generate random crops for the batch of images
            crop = jax.random.choice(crop_size_rng, jnp.array([image_size//4,image_size//2, image_size]), p=jnp.array([2/5*(1-base_p), 3/5*(1-base_p),base_p]))
            crop_size = (crop, crop)
            batch = random_crop_batch(crop_rng, batch, crop_size)
