...
def train_step(state: EMATrainState, x, discriminator_state: EMATrainState):
    def loss_fn(params):
        reconstruct = state.apply_fn({'params': params}, x)

        fake_logit, _ = discriminator_state.apply_fn({'params':discriminator_state.params,'batch_stats':discriminator_state.batch_stats},reconstruct,mutable=['batch_stats'])
        gan_loss = -fake_logit.mean()
...
if __name__ == "__main__":
    ...
    with tqdm(total=trainer_configs['total_steps']) as pbar:
        ...
        for steps in range(finished_steps + 1, 1000000):
            ...
            batch = shard(batch)
            state, metrics = train_step(state, batch, discriminator_state)
