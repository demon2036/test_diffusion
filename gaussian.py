...
def create_state(rng, model_cls, input_shape, learning_rate, train_state, print_model=True, *args,
                 **kwargs):
...
class test:
    ...
    def model_predictions(self, params, x, t, *args, **kwargs):
        # print(params)
        # model_out = self.model.apply({"params": params}, x)
        # model_out =self.state.apply_fn({'params':self.state.params},x)
...
def generator(batch_size=32):
    ...
    d = get_dataloader(batch_size, '/home/john/datasets/celeba-128/celeba-128', cache=True)
    while True:
        for data in d:
            # x, y = data
            x = data
...
if __name__ == "__main__":
    ...
    c = test(model_kwargs=model_kwargs, loss='l2', image_size=64, timesteps=1000, sampling_timesteps=999
             , beta_schedule='cosine')
    dl = generator(batch_size=16)
    state = create_state(rng=key, model_cls=Unet, input_shape=input_shape, learning_rate=5e-5,
                         train_state=TrainState, **model_kwargs)

    model_ckpt = {'model': state, 'steps': 0}
    save_path = './chkepoints'
    ...
    with tqdm(total=1000000) as pbar:
        ...
        for steps in range(finished_steps + 1, 1000000):
            ...
            if steps % 10 == 0:
                ...
                c.set_state(unreplicate_state)
                sample = c.sample(key, unreplicate_state.params, batch_size=1)
                c.state = None
